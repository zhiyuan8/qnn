...
def convert_args_dict_to_namespace(args: dict, converter_frontend_argparser: argparse.ArgumentParser) -> argparse.Namespace:
    ...
    if not args.get('input_network'):
        log_error("required 'input_network' key is not found in args dictionary")
        sys.exit(-1)
    namespace_instance = converter_frontend_argparser.parse_args(['--input_network', args['input_network']])
